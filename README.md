...
- Summary tables have no separate search box or density menu. Long descriptions wrap automatically, and tables above 25 rows use pagination. Overview's compact tables wrap to fit their panels, with all numeric columns visible. Short-label grids elsewhere retain native sorting/resizing (sorting applies to the displayed page). **View details** is the keyboard-accessible row selector, including for wrapped tables. Search remains available in Records and review tools. A new grid/chart click takes precedence over an earlier keyboard choice.
...
- Overview is one landing page: report context and last successful update, four headline measures, then side-by-side **Submission trend** and **Type of Intervention Sort** panels. Each panel has a **Chart / Table** switch and builds only the selected representation. The intervention table shows Intervention, Submissions and Share (%); the denominator is all filtered submissions. Monthly trend tables and charts use identical counts, including zero-submission months between recorded months. Raw names and CPV approvals are unchanged.
- **Explore further** links to People & Visits, Locations, Disability and CPV Work without clearing report filters. People & Visits contains the age, visit-history and overall disability-prevalence breakdowns previously nested in Overview; Locations contains camp/helpdesk coverage. Findings and period comparison are below the Overview panels. Other repeated chart/table breakdowns now use one representation switch instead of stacking duplicate content.
- On narrow screens the Overview panels stack vertically and headline cards use a two-by-two layout. Wrapped HTML tables have captions and row/column header associations. Desktop and 390-pixel mobile previews were checked with synthetic data for this redesign; no live Kobo records were used. The last data-update timestamp describes the loaded snapshot, not a guarantee of instantaneous streaming.
